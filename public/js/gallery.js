const createGallery = (gallery) => {
  const { title, url, thumbnailUrl, id } = gallery;
...
const showPage = () => {
  ...
  let endElementNumber = Math.min(
    startElementNumber + amount,
    galleryList.length - 1,
  );

  galleryList
    .slice(startElementNumber, endElementNumber)
    .forEach((d) =>
      galleries.insertAdjacentHTML('afterbegin', createGallery(d)),
    );
